...
xmax = max(Nodes_Coord(1, :));
ymax = max(Nodes_Coord(2, :));

for i = 1:length(Nodes_Coord)
    if Nodes_Coord(3, i) == -thickness
    	boundary_z = [boundary_z i];
    end
    if Nodes_Coord(1, i) == xmax || Nodes_Coord(2, i) == ymax || Nodes_Coord(1, i) == 0 || Nodes_Coord(2, i) == 0
        boundary_p = [boundary_p i];
    end
%     if Nodes_Coord(1, i) == 0
%         boundary_x = [boundary_x i];
%     end
...
e_boundary_p_nset.node = cell(1, length(boundary_p)); 
e_boundary_p_nset.Attributes.name = 'FixedSide_p';
for i = 1 : length(e_boundary_p_nset.node)
    e_boundary_p_nset.node{i}.Attributes.id = num2str(boundary_p(i)); 
end
...
e_new.febio_spec.Mesh.NodeSet{2} = e_boundary_p_nset;
...
%e_new.febio_spec.Boundary.bc{2}.Attributes.name = 'FixedEffectiveFluidPressure';
%e_new.febio_spec.Boundary.bc{2}.Attributes.node_set = 'FixedSide_p'; 
%e_new.febio_spec.Boundary.bc{2}.dofs.Text = 'p';
...
e_new.febio_spec.Rigid.rigid_constraint{2}.value.Attributes.lc = '2'; 
