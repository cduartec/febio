...
%basic_nodes_1 = (0 : elemsize/3 : w_seg/2.5/2);
%basic_nodes_2 = (w_seg/2.5/2 + elemsize : elemsize : w_seg/2.4);
%basic_nodes = [basic_nodes_1,basic_nodes_2];
...
%basic_plane_1 = (0 : elemsize/3 : l_seg/2.5/2);
%basic_plane_2 = (l_seg/2.5/2 + elemsize : elemsize : l_seg/2.4);
%basic_plane = [basic_plane_1,basic_plane_2];
...
%basic_layers_1 = (0 : elemsize : t_seg/2.5/2);
%basic_layers_2 = (t_seg/2.5/2 + elemsize : elemsize/3 : t_seg/2.4);
%basic_layers = [basic_layers_1,basic_layers_2];
...
Sp_1_x_num=round(Sp_radius*pi/4/Sp_radian)+1;
Sp_1_y_num=round(Sp_radius*pi/4/Sp_radian)+1;
...
for i=1:length(Sp_nodes_ori)
    Sp_nodes(i).coord = Sp_radius * Sp_nodes_ori(i).coord + [0,0,Sp_radius]; 
...
for i = 1 : num_Nodes_sphere 
    e_test.febio_spec.Mesh.Nodes.node{num_Nodes_solid + i}.Text = [num2str(sphere_node(i, 1)), ',', num2str(sphere_node(i, 2)), ',', num2str(sphere_node(i, 3))];
...
for i = 1 : num_Elems_sphere 
    ...
    elemText(end) = ''; 
    e_test.febio_spec.Mesh.Elements{2}.elem{i}.Text = elemText; 
    e_test.febio_spec.Mesh.Elements{2}.elem{i}.Attributes.id = num2str(num_Elems_solid + i); 
end   

e_test.febio_spec.Mesh.Elements{2}.Attributes.name = 'indenter'; 
...
e_new.febio_spec.MeshDomains.SolidDomain{1}.Attributes.name = 'indenter'; 
e_new.febio_spec.MeshDomains.SolidDomain{1}.Attributes.mat = 'Indenter'; 
